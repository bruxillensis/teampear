...
T* createProfessor<T>::createNewProfessor(string professorName, vector<vector<string>>& csv, int begRow){
    ...
    return createNewPubProfessor(professorName,csv,begRow);
...
template <class T>
pubProfessor* createProfessor<T>::createNewPubProfessor(string professorName, vector<vector<string>>& csv,int begRow){
    ...
    while (currLine < entries){ //Search every entry of csv
        //If a line begins with the beginning number of the member create an entry for that publication
        if (csv[currLine][0] = begRow){
